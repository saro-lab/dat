...
fn unit(
    fail_certificate: &DatCertificate,
    cid: u64,
    ...
    let new_certificate = DatCertificate::generate(
        cid,
...
fn test() {
    ...
    let fail_certificate = &DatCertificate::generate(
        192874,
        now_unix_timestamp() - 10,
        200,
        100,
    ...
    signature_alg_arr.iter().for_each(|sign_alg| {
        crypto_alg_arr.iter().for_each(|crypto_alg| {
            (1..20).for_each(|i| {
                ...
                assert!(
                    unit(fail_certificate, i, *sign_alg, *crypto_alg, plain, secure)
                        .map_err(|e| {
                            println!("@{:?}", e);
                            e
                        })
